...
def process_element(element, grids, bbox, floors, grid_size, total_elements, current_element):
    ...
    if element.is_a() in ['IfcWall', 'IfcWallStandardCase', 'IfcColumn']:
        element_type = 'wall'
    elif element.is_a() == 'IfcDoor':
        element_type = 'door'
    elif element.is_a() in ['IfcStair', 'IfcStairs']:
        element_type = 'stair'
    else:
...
def mark_cells(triangle, grids, bbox, floors, grid_size, element_type):
    ...
    min_z = min(p[2] for p in triangle) + (-0.1 if element_type == 'stair' else 0.3)
    max_z = max(p[2] for p in triangle) - 0.3
...
def visualize_grids(grids, floors):
    ...
    for floor_index, (ax, floor) in enumerate(zip(axs.flat, floors)):
        for element_type in ['empty', 'wall', 'stair', 'door']:  # Order matters for visibility
            y, x = np.where(grids[floor_index] == element_type)
            ax.scatter(x, y, c=colors[element_type], marker='s', s=9, edgecolors='none')
...
def export_grids(grids, bbox, floors, filename):
    data = {
...
# Usage
ifc_file_path = 'Schependom.ifc'  # Replace with your IFC file path
grids, bbox, floors = create_navigation_grid(ifc_file_path, grid_size=0.25)
visualize_grids(grids, floors)

# Export the grids
export_grids(grids, bbox, floors, 'bim_grids.json')